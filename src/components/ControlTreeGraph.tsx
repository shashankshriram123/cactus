  ...
  React.useImperativeHandle(ref, () => ({
    ...
    useEffect(() => {
        ...
        const foldSelected = () => {
                ...
                // Hide all descendant branches AND their nodes
                getDescendantBranches(localSelectedNode, liveGraph.current.branches).forEach(b => {
                    b.isHidden = true; // Hide branch (edges)
                    b.nodes.forEach(n => n.isHidden = true); // Hide all nodes in branch
                });
        ...
        const collapseSelected = () => {
                ...
                const branch = localSelectedNode.branch;
                const nodesToHide = branch.nodes.filter(node => node.y < localSelectedNode!.y);
                
                // Hide all nodes above the selected node in this branch
                nodesToHide.forEach(node => {
                    node.isHidden = true;
                    // Hide all descendant branches of these nodes
                    getDescendantBranches(node, liveGraph.current.branches).forEach(b => {
                    b.isHidden = true;
                    b.nodes.forEach(n => n.isHidden = true); // Hide all nodes in descendant branches
                    });
                });
                
                // Mark the selected node as the new head
                localSelectedNode.isHead = true;
                relayout();
                };
    ...
    return (
        <div ref={canvasContainerRef} className="canvas-container">
            ...
            <div className="controls-overlay">
                ...
                <button 
                        onClick={() => console.log("Delete Node Button Click Registered")}
                ...
                <button 
                    onClick={() => {
                        console.log("Delete Node button clicked"); // Debug log
                        callCanvasFunc('deleteNode');
                    }}
                    disabled={buttonStates.deleteNode} 
                    className="btn-danger"
                    >
                    Delete Node
                    </button>
